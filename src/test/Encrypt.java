 ...
 */
/**
 * 完成数据的加密功能,加密所用密钥由和教育平台提供。
 * 
 * @author hefte *
 */
public class Encrypt {
    private final String KEY = "xxtwebService";

    public static void main(String[] args) {
        Encrypt encrypt = new Encrypt();
        String encodeStr;
        try {
            encodeStr = encrypt.authcode("a", "ENCODE",
                    "ce6fd15cb469d2dc7961dd575c5ac9cb", 0);
            String decodeStr = encrypt.authcode(encodeStr, "DECODE",
                    "ce6fd15cb469d2dc7961dd575c5ac9cb");
     ...
     * @param string
     *            string 原文或者密文
     * @paramstringoperation 操作(ENCODE|DECODE),默认为DECODE
     * @param string
     *            key
     * @param int expiry
     * @return string *
     ...
     *          'key'); // b(abc) * a = authcode('abc', 'ENCODE', 'key', 3600);
     *          b = authcode('abc', 'DECODE', 'key'); // 在一个小时内,b(abc),否则 b 为空
     */
    public String authcode(String string, String operation) throws Exception {
    ...
    private String base64_decode(String input) {
        try {
            return new String(Base64.decodeBase64(input.getBytes()), "iso-8859-1");
        } catch (Exception e) {
            return e.getMessage();
        }
    }
